...
import { avatarUrl, deleteUploadedFile } from "../middleware/upload.middleware";
...
export const getProfile = async (
  ...
  const user = await User.findById(req.user.sub).populate(
    "enrolledCourses",
    "title slug thumbnail level category",
  );
  if (!user) {
...
export const updateProfile = async (
  ...
  allowed.forEach((f) => {
    if (req.body[f] !== undefined) updates[`profile.${f}`] = req.body[f];
  });
  ...
  logSecurityEvent("profile_updated", user._id.toHexString(), ip(req), {
...
export const uploadAvatarHandler = async (
  ...
  user.profile.avatarUrl = avatarUrl(req.file.filename);
  await user.save({ validateBeforeSave: false });

  if (oldAvatar) deleteUploadedFile(oldAvatar);

  logSecurityEvent("avatar_updated", user._id.toHexString(), ip(req), {});
  res.status(200).json({ message: "Avatar updated", profile: user.profile });
};
...
export const changePassword = async (
  ...
  if (!policy.valid) {
    res
      .status(400)
      .json({ error: "Password policy violation", details: policy.errors });
    return;
  }
...
export const exportProfile = async (
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=gyankosh-profile.json",
  );
  res.status(200).json({
