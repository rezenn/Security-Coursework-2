...
export const listUsers = async (req: Request, res: Response): Promise<void> => {
  ...
  const [users, total] = await Promise.all([
    User.find(filter).select("-password -mfa.secret -mfa.backupCodes -passwordHistory -activeRefreshTokens").sort({ createdAt: -1 }).skip(skip).limit(parseInt(limit as string)),
...
export const toggleUserActive = async (req: Request, res: Response): Promise<void> => {
  ...
  logSecurityEvent("admin_toggle_user", req.user.sub, ip(req), { targetId: req.params.id, isActive: user.isActive });
...
export const deleteUser = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!user) { res.status(404).json({ error: "User not found" }); return; }
  logSecurityEvent("admin_delete_user", req.user.sub, ip(req), { targetId: req.params.id });
...
export const getStats = async (_req: Request, res: Response): Promise<void> => {
  ...
  const [totalUsers, totalCourses, totalTransactions, revenueAgg] = await Promise.all([
    User.countDocuments(),
    Course.countDocuments(),
    Transaction.countDocuments({ status: "completed" }),
    Transaction.aggregate([{ $match: { status: "completed" } }, { $group: { _id: null, total: { $sum: "$amountCents" } } }]),
  ]);
