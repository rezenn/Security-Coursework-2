...
import {
  BookOpen, CreditCard, ShieldCheck, ShieldAlert,
  ArrowRight, Clock,
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const displayName = profile?.profile?.firstName || profile?.username || "there";
  const totalSpentPaisa = transactions
    ...
    <div className="max-w-5xl mx-auto">
      ...
      {!mfaEnabled && (
        <div className="flex items-start gap-3 bg-amber-500/8 border border-amber-500/25 rounded-xl p-4 mb-7">
          <ShieldAlert size={17} className="text-amber-400 mt-0.5 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-amber-300 text-sm font-medium">Your account is not fully secured</p>
            <p className="text-amber-400/70 text-xs mt-0.5">
              Enable two-factor authentication to protect against unauthorised access.
            </p>
          </div>
          <Link
            href="/mfa-setup"
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Courses</p>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-white">
            Rs. {(totalSpentPaisa / 100).toFixed(0)}
          </p>
          <p className="text-xs text-slate-500 mt-0.5">via Khalti</p>
        </div>

        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Security</p>
            <div className={clsx("w-7 h-7 rounded-lg flex items-center justify-center",
              mfaEnabled ? "bg-emerald-600/20" : "bg-amber-600/20"
            )}>
              <ShieldCheck size={13} className={mfaEnabled ? "text-emerald-400" : "text-amber-400"} />
            </div>
          </div>
          <p className="text-2xl font-bold text-white">{mfaEnabled ? "On" : "Off"}</p>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/courses" className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 transition-colors">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
            {recentTx.map((tx: any, i: number) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white truncate">{tx.course?.title || "Unknown course"}</p>
