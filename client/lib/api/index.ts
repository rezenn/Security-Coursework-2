...
export const authApi = {
  ...
  requestReset: (email: string, captchaToken?: string) =>
    api
      .post(API.AUTH.REQUEST_RESET, { email, captchaToken })
      .then((r) => r.data),
  resetPasswordToken: (
    token: string,
    password: string,
    ...
    api
      .post(API.AUTH.RESET_PASSWORD(token), { password, captchaToken })
      .then((r) => r.data),
...
export const adminApi = {
  ...
  toggleUser: (id: string) =>
    api.patch(API.ADMIN.TOGGLE_USER(id), {}).then((r) => r.data),
